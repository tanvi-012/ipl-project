...
export default function TeamDashboard() {
  ...
  useEffect(() => {
    axios.get('http://localhost:5000/team/dc')
      .then(response => {
        const selected = response.data || [];
        setSelectedPlayers(selected);
      })
      .catch(error => {
        console.error('Error fetching team/dc:', error);
      });

    axios.get('http://localhost:5000/user/selectedteam/dc')
      .then(response => {
      ...
      .catch(error => {
        console.error('Error fetching user/selectedteam/dc:', error);
      });
  }, []);
