...
export default function TeamDashboard() {
  ...
  const [teamName, setTeamName] = useState('lsg');
  ...
  useEffect(() => {
    axios.get('http://localhost:5000/team/lsg')
      .then(response => {
        const selected = response.data || [];
        setSelectedPlayers(selected);
      })
      .catch(error => {
        console.error('Error fetching team/lsg:', error);
      });

    axios.get('http://localhost:5000/user/selectedteam/lsg')
      .then(response => {
      ...
      .catch(error => {
        console.error('Error fetching user/selectedteam/lsg:', error);
      });
  }, []);
